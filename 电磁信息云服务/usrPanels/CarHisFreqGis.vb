...
Public Class CarHisFreqGis
    ...
    Private Sub CarHisFreqGis_Load(sender As Object, e As EventArgs) Handles Me.Load
    ...
    End Sub
    Private Sub GetCarDeviceList()
        Dim th As New Thread(Sub()
                                 Me.Invoke(Sub() Label2.Visible = True)
                                 Dim np As normalResponse = GetServerNp("func=GetCarDeviceList")
                                 Me.Invoke(Sub() Label2.Visible = False)
                                 If np.result = False Then
                                     Return
                                 End If
                                 Try
                                     Dim dt As DataTable = JsonConvert.DeserializeObject(np.data.ToString, GetType(DataTable))
                                     If IsNothing(dt) Then Return
                                     If dt.Rows.Count = 0 Then Return
                                     carList = New List(Of deviceStu)
                                     For Each row As DataRow In dt.Rows
                                         Dim car As New deviceStu
                                         car.DeviceID = row("DeviceId")
                                         car.Name = row("DeviceNickName")
                                         car.OnlineTime = row("OnlineTime")
                                         car.Lng = row("Lng")
                                         car.Lat = row("Lat")
                                         carList.Add(car)
                                     Next
    ...
    Private Sub ThGetBusHisFreqList()
        Try
            ...
            Catch ex As Exception
                MsgBox("结束时间格式不正确")
            End Try
            Dim param As String = "func=GetBusHisFreqList&startTime=" & startTime & "&endTime=" & endTime & "&deviceId=" & selectDeviceId
...
#End Region
    ...
    Private Sub CarHisFreqGis_Resize(sender As Object, e As EventArgs) Handles Me.Resize
